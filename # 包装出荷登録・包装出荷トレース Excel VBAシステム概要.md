...
- **包装出荷登録**  
  出荷データの登録・管理を行うExcel VBAマクロブック。  
  主に出荷予定・実績の入力、在庫引当、出荷先情報の管理などを担う。

- **包装出荷トレース**  
  出荷済み製品のトレース（追跡）や検索を行うExcel VBAマクロブック。  
...
- テーブル名の先頭「LIBWMF17」「LIBSMF17」等はライブラリ（スキーマ）名です。
- 各テーブルのカラムはSQL文やJOIN句から抽出した代表的なものです。
- 登録系（Reg_）はINSERT/UPDATEも行いますが、トレース系（Trc_）は主にSELECT（参照）用途です。
- さらに詳細なカラムや業務ロジックが必要な場合は、個別に調査・追記可能です。
